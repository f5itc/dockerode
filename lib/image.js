...
Image.prototype.push = function(opts, callback, auth) {
  ...
  // If image name has a registry prefix, add it to the registry option
  if (name.indexOf('/') > -1) {
    var parts = name.split('/');
    opts.registry = opts.registry || parts[0];
    name = parts[1];
  }

  // If image name has a tag suffix, add it to the tag option
  if (name.indexOf(':') > -1) {
    var parts = name.split(':');
    name = parts[0];
    opts.tag = opts.tag || parts[1];
  }
